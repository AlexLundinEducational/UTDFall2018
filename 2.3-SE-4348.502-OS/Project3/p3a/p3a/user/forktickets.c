...
main(int argc, char *argv[]){
	...
	int pid_par = getpid();
	int tickets = 5;
	printf(1, "PID on user side: %d", pid_par);
	check(settickets(tickets) == 0, "settickets");
	 ...
	 if(fork() == 0){
       ...
       if (st.pid[i] == pid_par){
		 ...
		 printf(1, "PID: %d ,parent has this many tickets: %d", pid_par, tickets_par);
		 printf(1, "\n");
		 printf(1, "PID: %d ,child has this many tickets: %d", pid_chd, tickets_chd);
		 printf(1, "\n");
